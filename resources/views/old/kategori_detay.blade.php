    ...
    <div class="breadcumb-wrapper" data-bg-src="{{ asset('frontend') }}/assets/img/bg/breadcumb-bg.jpg">
        <div class="container">
            <div class="breadcumb-content">
                <h1 class="breadcumb-title">{{ $kategoriAdi->kategori_adi }}</h1>
                <ul class="breadcumb-menu">
                    <li><a href="{{ url('/') }}">Anasayfa</a></li>
                    <li>Blog Kategorileri</li>
                ...
                <div class="col-xxl-8 col-lg-7">
                    @foreach ($blogpost as $blogposts)
                        <div class="th-blog blog-single has-post-thumbnail">
                            <div class="blog-img"><a href="{{ url('blog/' . $blogposts->id . '/' . $blogposts->url) }}"><img
                                        src="{{ !empty($blogposts->resim) ? url($blogposts->resim) : url('uploads/bloglar/resim-yok.jpg') }}"
                                        alt="Blog Image"></a>
                            </div>
                            <div class="blog-content">
                                <div class="blog-meta"><a class="author" href="javascript:void(0);"><i
                                        ...
                                        Michel</a> <a href="javascript:void(0);"><i
                                            class="fa-solid fa-calendar-days"></i>{{ $blogposts->created_at->tz('Europe/Istanbul')->format('d.m.Y - H:i') }}</a>
                                    <a href="{{ url('blog/' . $blogposts->id . '/' . $blogposts->url) }}"><i
                                            class="fa-solid fa-tags"></i>Construction</a>
                                </div>
                                <h2 class="blog-title"><a
                                        href="{{ url('blog/' . $blogposts->id . '/' . $blogposts->url) }}">{{ $blogposts->baslik }}
                                    </a></h2>
                                <p class="blog-text">{!! Str::limit($blogposts->aciklama, 200, '...') !!}</p><a
                                    href="{{ url('blog/' . $blogposts->id . '/' . $blogposts->url) }}"
